...
def audit_dossier():
    ...
    # Validate Karma, Nuyen, Initiation Grade math
    lifetime_karma = totals.get("Lifetime_Karma", 0)
    current_karma = totals.get("Karma", 0)
    nuyen = totals.get("Nuyen", 0)
    initiation_grade = totals.get("Initiation_Grade", 0)
    coven_loyalty = totals.get("Coven_Loyalty", 0)
    ...
    if initiation_grade < 0 or initiation_grade > 9:
        warnings.append(f"Unusual Initiation Grade detected ({initiation_grade})!")
...
def print_audit_report(report):
    ...
    print(f" Initiation Grade: Grade {report['initiation_grade']}")
    print(f" Coven Loyalty: {report['coven_loyalty']}")
    ...
    if report['warnings']:
        ...
        print(" [OK] All Karma, Nuyen, and Initiation ledger balances are mathematically consistent.")
    print("=" * 65 + "\n")
